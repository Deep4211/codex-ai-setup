...
description: Automated coordination, formatting, and learning from Codex operations
...
**Required:**
- Claude Flow CLI installed (`npm install -g ruflo@latest`)
- Codex with hooks enabled
- `.codex/hooks.json` with hook configurations
...
This creates:
- `.codex/hooks.json` with pre-configured hooks
- Hook command documentation in `.codex/hooks/`
...
npx ruflo@latest hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
npx ruflo@latest hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
npx ruflo@latest hook session-end --session-id "dev-session" --export-metrics
```
...
```bash
npx ruflo@latest hook pre-edit [options]
...
Examples:
  npx ruflo@latest hook pre-edit --file "src/auth/login.js"
  npx ruflo@latest hook pre-edit -f "config/db.js" --validate-syntax
  npx ruflo@latest hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
npx ruflo@latest hook pre-bash --command <cmd>
...
Examples:
  npx ruflo@latest hook pre-bash -c "rm -rf /tmp/cache"
  npx ruflo@latest hook pre-bash --command "docker build ." --estimate-resources
```
...
```bash
npx ruflo@latest hook pre-task [options]
...
Examples:
  npx ruflo@latest hook pre-task --description "Implement user authentication"
  npx ruflo@latest hook pre-task -d "Continue API dev" --load-memory
  npx ruflo@latest hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
npx ruflo@latest hook pre-search --query <query>
...
Examples:
  npx ruflo@latest hook pre-search -q "authentication middleware"
```
...
```bash
npx ruflo@latest hook post-edit [options]
...
Examples:
  npx ruflo@latest hook post-edit --file "src/components/Button.jsx"
  npx ruflo@latest hook post-edit -f "api/auth.js" --memory-key "auth/login"
  npx ruflo@latest hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
npx ruflo@latest hook post-bash --command <cmd>
...
Examples:
  npx ruflo@latest hook post-bash -c "npm test" --update-metrics
```
...
```bash
npx ruflo@latest hook post-task [options]
...
Examples:
  npx ruflo@latest hook post-task --task-id "auth-implementation"
  npx ruflo@latest hook post-task -t "api-refactor" --analyze-performance
  npx ruflo@latest hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
npx ruflo@latest hook post-search --query <query> --results <path>
...
Examples:
  npx ruflo@latest hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
npx ruflo@latest hook mcp-initialized --swarm-id <id>
...
```bash
npx ruflo@latest hook agent-spawned --agent-id <id> --type <type>
...
```bash
npx ruflo@latest hook task-orchestrated --task-id <id>
...
```bash
npx ruflo@latest hook neural-trained --pattern <name>
...
```bash
npx ruflo@latest hook memory-sync --namespace <ns>
...
```bash
npx ruflo@latest hook session-start --session-id <id>
...
```bash
npx ruflo@latest hook session-restore --session-id <id>
...
Examples:
  npx ruflo@latest hook session-restore --session-id "swarm-20241019"
  npx ruflo@latest hook session-restore -s "feature-auth" --restore-memory
```
...
```bash
npx ruflo@latest hook session-end [options]
...
Examples:
  npx ruflo@latest hook session-end --session-id "dev-session-2024"
  npx ruflo@latest hook session-end -s "feature-auth" --export-metrics --generate-summary
  npx ruflo@latest hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
npx ruflo@latest hook notify --message <msg>
...
Examples:
  npx ruflo@latest hook notify -m "Task completed" --level info
  npx ruflo@latest hook notify -m "Critical error" --level error --broadcast
```
...
Edit `.codex/hooks.json` to configure hooks:
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx ruflo@latest hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx ruflo@latest hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx ruflo@latest hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx ruflo@latest hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo@latest hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo@latest hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo@latest hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo@latest hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo@latest hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo@latest hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo@latest hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo@latest hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo@latest hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
npx ruflo@latest hook pre-task --description "Build REST API"
...
```javascript
// Hook command
npx ruflo@latest hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
npx ruflo@latest hook session-end --session-id "dev-2024"
...
for FILE in $FILES; do
  ...
  npx ruflo@latest hook pre-edit --file "$FILE" --validate-syntax
  ...
  npx ruflo@latest hook post-edit --file "$FILE" --auto-format
done
...
# Run quality checks
npx ruflo@latest hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(npx ruflo@latest metrics score --format json | jq -r '.truth_score')
...
npx ruflo@latest hook pre-task \
  --description "Implement user authentication API" \
  --auto-spawn-agents \
  --load-memory

# STEP 2: Work begins - pre-edit validation
npx ruflo@latest hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax
...
npx ruflo@latest hook post-edit \
  --file "api/auth.js" \
...
npx ruflo@latest hook post-task \
  --task-id "auth-api" \
  --analyze-performance \
  --store-decisions \
  --export-learnings
...
npx ruflo@latest hook session-restore \
...
npx ruflo@latest hook session-start --session-id "fullstack-feature"
...
# Backend work
npx ruflo@latest hook pre-edit --file "api/profile.js"
# ... implement backend ...
npx ruflo@latest hook post-edit \
  --file "api/profile.js" \
...
npx ruflo@latest hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
npx ruflo@latest hook post-edit \
...
npx ruflo@latest hook session-start --session-id "debug-memory-leak"
...
npx ruflo@latest hook pre-search --query "EventEmitter"
# ... search executes ...
npx ruflo@latest hook post-search \
  --query "EventEmitter" \
  --cache-results
...
npx ruflo@latest hook pre-task --description "Analyze code complexity"
# ... analysis ...
npx ruflo@latest hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  npx ruflo@latest hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  npx ruflo@latest hook post-edit \
    --file "$file" \
...
npx ruflo@latest hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .claude-flow/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
npx ruflo@latest hook validate-config
```
...
#### Hooks Not Executing
- Verify `.codex/hooks.json` syntax
...
- `npx ruflo@latest init --hooks` - Initialize hooks system
- `npx ruflo@latest hook --list` - List available hooks
- `npx ruflo@latest hook --test <hook>` - Test specific hook
- `npx ruflo@latest memory usage` - Manage memory
- `npx ruflo@latest agent spawn` - Spawn agents
