...
- Claude Flow installed (`npx ruflo@latest`)
...
npx ruflo@latest truth

# Run verification check
npx ruflo@latest verify check

# Verify specific file with custom threshold
npx ruflo@latest verify check --file src/app.js --threshold 0.98

# Rollback last failed verification
npx ruflo@latest verify rollback --last-good
```
...
npx ruflo@latest truth --period 7d

# View scores for specific agent
npx ruflo@latest truth --agent coder --period 24h

# Find files/tasks below threshold
npx ruflo@latest truth --threshold 0.8
```

**Output Formats:**
```bash
# Table format (default)
npx ruflo@latest truth --format table

# JSON for programmatic access
npx ruflo@latest truth --format json

# CSV for spreadsheet analysis
npx ruflo@latest truth --format csv

# HTML report with visualizations
npx ruflo@latest truth --format html --export report.html
```

**Real-time Monitoring:**
```bash
# Watch mode with live updates
npx ruflo@latest truth --watch

# Export metrics automatically
npx ruflo@latest truth --export .claude-flow/metrics/truth-$(date +%Y%m%d).json
```
...
```bash
# Verify single file
npx ruflo@latest verify check --file src/app.js

# Verify directory recursively
npx ruflo@latest verify check --directory src/

# Verify with auto-fix enabled
npx ruflo@latest verify check --file src/utils.js --auto-fix
...
npx ruflo@latest verify check --task task-123

# Verify with custom threshold
npx ruflo@latest verify check --task task-456 --threshold 0.99

# Verbose output for debugging
npx ruflo@latest verify check --task task-789 --verbose
```

**Batch Verification:**
```bash
# Verify multiple files in parallel
npx ruflo@latest verify batch --files "*.js" --parallel

# Verify with pattern matching
npx ruflo@latest verify batch --pattern "src/**/*.ts"

# Integration test suite
npx ruflo@latest verify integration --test-suite full
```
...
npx ruflo@latest verify check --json > verification.json
...
npx ruflo@latest verify rollback --to-commit abc123

# Interactive rollback with preview
npx ruflo@latest verify rollback --interactive
```

**Smart Rollback:**
```bash
# Rollback only failed files (preserve good changes)
npx ruflo@latest verify rollback --selective

# Rollback with automatic backup
npx ruflo@latest verify rollback --backup-first

# Dry-run mode (preview without executing)
npx ruflo@latest verify rollback --dry-run
```
...
```bash
# JSON report
npx ruflo@latest verify report --format json

# HTML report with charts
npx ruflo@latest verify report --export metrics.html --format html

# CSV for data analysis
npx ruflo@latest verify report --format csv --export metrics.csv

# Markdown summary
npx ruflo@latest verify report --format markdown
```

**Time-based Reports:**
```bash
# Last 24 hours
npx ruflo@latest verify report --period 24h

# Last 7 days
npx ruflo@latest verify report --period 7d

# Last 30 days with trends
npx ruflo@latest verify report --period 30d --include-trends

# Custom date range
npx ruflo@latest verify report --from 2025-01-01 --to 2025-01-31
```
...
npx ruflo@latest verify dashboard

# Custom port
npx ruflo@latest verify dashboard --port 8080

# Export dashboard data
npx ruflo@latest verify dashboard --export

# Dashboard with auto-refresh
npx ruflo@latest verify dashboard --refresh 5s
```
...
npx ruflo@latest verify check --threshold 0.99

# Lenient mode (90% acceptable)
npx ruflo@latest verify check --threshold 0.90

# Set default threshold
npx ruflo@latest config set verification.threshold 0.98
```
...
verify:
  stage: test
  script:
    - npx ruflo@latest verify check --threshold 0.95 --json > verification.json
    - |
      score=$(jq '.overallScore' verification.json)
...
npx ruflo@latest swarm --verify --threshold 0.98

# Hive Mind with auto-rollback
npx ruflo@latest hive-mind --verify --rollback-on-fail

# Training pipeline with verification
npx ruflo@latest train --verify --threshold 0.99
```
...
npx ruflo@latest pair --verify --real-time

# Pair with custom threshold
npx ruflo@latest pair --verify --threshold 0.97 --auto-fix
```
...
npx ruflo@latest verify watch --directory src/

# Watch with auto-fix
npx ruflo@latest verify watch --directory src/ --auto-fix

# Watch with notifications
npx ruflo@latest verify watch --notify --threshold 0.95
```
...
npx ruflo@latest verify install-hook --pre-commit

# .git/hooks/pre-commit example:
#!/bin/bash
npx ruflo@latest verify check --threshold 0.95 --json > /tmp/verify.json

score=$(jq '.overallScore' /tmp/verify.json)
if (( $(echo "$score < 0.95" | bc -l) )); then
  echo "❌ Verification failed with score: $score"
  echo "Run 'npx ruflo@latest verify check --verbose' for details"
  exit 1
fi
...
npx ruflo@latest truth --verbose --threshold 0.0

# Check specific criteria
npx ruflo@latest verify check --verbose

# View agent-specific issues
npx ruflo@latest truth --agent <agent-name> --format json
```
...
npx ruflo@latest verify rollback --history
...
```bash
# Increase timeout
npx ruflo@latest verify check --timeout 60s

# Verify in batches
npx ruflo@latest verify batch --batch-size 10
```
...
- `npx ruflo@latest pair` - Collaborative development with verification
- `npx ruflo@latest train` - Training with verification feedback
- `npx ruflo@latest swarm` - Multi-agent coordination with quality checks
- `npx ruflo@latest report` - Generate comprehensive project reports
