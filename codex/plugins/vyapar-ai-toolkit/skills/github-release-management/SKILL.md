...
npx ruflo@latest github release-create \
  --version "2.0.0" \
  --build-artifacts \
...
npx ruflo@latest sparc pipeline "Release v2.0.0 with full validation"
```
...
npx ruflo@latest github changelog \
  --prs "$PRS" \
  --commits "$COMMITS" \
  --from v1.0.0 \
  --to HEAD \
...
npx ruflo@latest github version-suggest \
  --current v1.2.3 \
  --analyze-commits \
  --check-compatibility \
  --suggest-pre-release
...
npx ruflo@latest github release-test \
...
npx ruflo@latest github release-deploy \
...
npx ruflo@latest github multi-release \
...
npx ruflo@latest github performance-test \
  --baseline v1.9.0 \
  --candidate v2.0.0 \
...
npx ruflo@latest github rollback-config \
  --triggers '{
    "error-rate": ">5%",
    "latency-p99": ">1000ms",
    "availability": "<99.9%",
...
npx ruflo@latest github release-compliance \
